...
const filteredSessions = useMemo(() => {
  ...
  // Traducción de estados - IGUAL que en Mis Sesiones
      ...
      <Link href={`/shot/sesion/${session.id}`} key={session.id} className="group">
        <div className="relative rounded-3xl overflow-hidden bg-black shadow hover:shadow-xl transition">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 p-5 text-white">
           <div className={`text-xs mt-2 inline-block px-3 py-1 rounded-full font-medium ${
  session.status?.toUpperCase() === 'ACTIVE' ? 'bg-green-600' : 
  session.status?.toUpperCase() === 'DRAFT' ? 'bg-amber-600' : 
  session.status?.toUpperCase() === 'PROCESSING' ? 'bg-blue-600' : 
  'bg-gray-600'
}`}>
