...
const getSessionDateValue = (session) => {
  if (session.sessionDate) {
    const t = new Date(session.sessionDate).getTime();
    if (!Number.isNaN(t)) return t;
  }

  const m = session.titleShort?.match(/(\d{1,2})\/(\d{1,2})/);
  if (m) {
...
const timeToMinutes = (time) => {
  if (!time) return 0;
  const [h, min] = String(time).slice(0, 5).split(':').map(Number);
  return (h || 0) * 60 + (min || 0);
};
...
export default function SesionesPage() {
  ...
  const sortedSessions = useMemo(() => {
    return [...filteredSessions].sort((a, b) => {
      const dateDiff = getSessionDateValue(b) - getSessionDateValue(a);
